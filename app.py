...
def show_products_table_header():
    print("+------+--------------------------------+--------------------------------+------------------+-------------+-----------------------------+")
    print(f"| {'ID':^4} | {_('name'):^30} | {_('description'):^30} | {_('quantity'):^16} | {_('price'):^11} | {_('category'):^27} |")
    print("+------+--------------------------------+--------------------------------+------------------+-------------+-----------------------------+")

def product_to_table_row(producto):
    category = _(producto[5]) if producto[5] in translations else producto[5]
    return f"| {producto[0]:^4} | {producto[1][:20]:^30} | {producto[2][:20]:^30} | {producto[3]:^16} | ${producto[4]:^10.2f} | {category[:17]:^27} |"
